...
export const productsRouter = router({
  list: publicProcedure
    .input(z.object({
      ...
      sort: z.enum(["newest", "price_asc", "price_desc", "views"]).optional(),
  ...
  bySlug: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const sellers = await getProductListings(product.id);
  ...
  createListing: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => createProductListing({
      ...input,
      priceUsd: input.priceUsd.toFixed(2),
      sellerId: ctx.user.id,
    }))),

  myListings: protectedProcedure.query(({ ctx }) => getUserProductListings(ctx.user.id)),
});
...
export const listingsRouter = router({
  search: publicProcedure
    .input(z.object({
      ...
      sort: z.enum(["price_asc", "price_desc", "newest", "views"]).optional(),
      page: z.number().int().min(1).default(1),
      pageSize: z.number().int().min(1).max(60).default(24),
    }).default({ page: 1, pageSize: 24 }))
  ...
  update: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => updateListing(input.id, ctx.user.id, {
      ...(input.priceUsd !== undefined ? { priceUsd: input.priceUsd.toFixed(2) } : {}),
...
export const cartRouter = router({
  ...
  add: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() =>
      addToCart(ctx.user.id, { listingId: input.listingId, productListingId: input.productListingId }, input.quantity),
    )),

  update: protectedProcedure
    .input(z.object({ cartItemId: z.number().int().positive(), quantity: z.number().int().min(0).max(99) }))
  ...
  checkout: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => checkoutCart(ctx.user.id, input.notes))),
});

// ─── Orders ───────────────────────────────────────────────────────────────────

export const ordersRouter = router({
  myPurchases: protectedProcedure.query(({ ctx }) => getBuyerOrders(ctx.user.id)),
  ...
  sellerReviews: publicProcedure
    .input(z.object({ sellerId: z.number().int().positive(), limit: z.number().int().min(1).max(50).default(20) }))
...
export const notificationsRouter = router({
  ...
  markRead: protectedProcedure
    .input(z.object({ ids: z.array(z.number().int().positive()).optional() }).default({}))
    .mutation(({ input, ctx }) => rethrow(() => markNotificationsRead(ctx.user.id, input.ids))),
});
...
export const bazaarRouter = router({
  ...
  mine: protectedProcedure.query(({ ctx }) => getUserBazaarListings(ctx.user.id)),
  ...
  create: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => createBazaarListing({
      ...input,
  ...
  remove: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => deleteBazaarListing(input.id, ctx.user.id))),
  ...
  addWant: protectedProcedure
    ...
    .mutation(({ input, ctx }) => rethrow(() => addWantListItem({
      ...input,
  ...
  removeWant: protectedProcedure
    .input(z.object({ id: z.number().int().positive() }))
    .mutation(({ input, ctx }) => rethrow(() => removeWantListItem(input.id, ctx.user.id))),
});

...
describe("Stripe marketplace checkout", () => {
  ...
  it("collects US shipping and creates a 30-minute card reservation", async () => {
    ...
    expect(session.expiresAt.getTime()).toBeGreaterThanOrEqual(before + 29 * 60_000);
    expect(session.expiresAt.getTime()).toBeLessThanOrEqual(before + 31 * 60_000);
  });

  it("refunds only the affected order amount", async () => {
    fetchMock.mockResolvedValueOnce(stripeResponse({ id: "re_partial" }));
    await expect(createRefund("ch_cart", 1234, "refund_order_42")).resolves.toBe(
      "re_partial"
    );

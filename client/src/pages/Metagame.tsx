...
export default function Metagame() {
  usePageMeta(
    "Metagame",
    "Pokémon TCG metagame analysis — top decks, usage stats and matchups."
  );
  const { data: decks, isLoading } = trpc.metagame.topDecks.useQuery({
    format: "TEF-CRI",
  });

  const top10 = decks?.slice(0, 10) ?? [];
  const top5 = decks?.slice(0, 5) ?? [];
  const otherShare = decks
    ? 100 - decks.slice(0, 5).reduce((s, d) => s + d.sharePercent, 0)
    : 0;

  const pieData = [
    ...top5.map(d => ({
      name: d.name,
      value: parseFloat(d.sharePercent.toFixed(1)),
    })),
    ...
    <div className="container py-8">
      ...
      <div className="mb-10">
        ...
        <p className="text-muted-foreground">
          Reference snapshot of competitive decks — verify the latest results at
          Limitless TCG
        </p>
        <a
        ...
        >
          Open live tournament data at Limitless TCG ↗
        </a>
      </div>
            ...
            <ResponsiveContainer width="100%" height={260}>
              ...
              >
                ...
                <YAxis
                  type="category"
                  dataKey="name"
                  tick={{ fill: "oklch(0.75 0.01 260)", fontSize: 11 }}
                  tickLine={false}
                  axisLine={false}
                  width={130}
                />
                ...
                <Bar dataKey="sharePercent" radius={[0, 4, 4, 0]}>
                  {top10.map((_, i) => (
                    <Cell
                      key={i}
                      fill={i < 3 ? RANK_COLORS[i] : "oklch(0.28 0.02 260)"}
                    />
                  ))}
                </Bar>
              </BarChart>
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    deck.rank === 1 &&
                      "bg-[oklch(0.78_0.18_85)] text-[oklch(0.12_0.015_260)]",
                    deck.rank === 2 &&
                      "bg-[oklch(0.75_0.05_260)] text-[oklch(0.12_0.015_260)]",
                ...
                {deck.imageUrl ? (
                  <img
                    src={deck.imageUrl}
                    alt={deck.name}
                    className="w-12 h-12 rounded-lg object-contain bg-muted border border-border shrink-0"
                  />
                ) : (
                ...
                <div className="w-32 shrink-0 hidden md:block">
                  ...
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(deck.sharePercent * 2, 100)}%`,
                      }}
                    />
                  </div>
                </div>
      ...
      <p className="text-xs text-muted-foreground text-center mt-6">
        This page is a clearly labeled cached reference, not a live ranking.
        Confirm current percentages and events on Limitless TCG.
      </p>
    </div>
  );
}

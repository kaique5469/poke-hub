...
const REPORT_REASONS = {
  suspected_counterfeit: "Suspected counterfeit products",
  misleading_listing: "Misleading store or listings",
...
export default function StorePage() {
  ...
  const storeQ = trpc.store.bySlug.useQuery(
    { slug },
  ...
  const report = trpc.store.report.useMutation({
    onSuccess: result => {
      toast.success(
        result.duplicate
          ? "Your existing report is already being reviewed."
          : "Safety report submitted. Thank you."
      );
      setReportOpen(false);
      setReportDetails("");
    },
  ...
  if (storeQ.isError || !data) {
    return (
      <div className="min-h-[50vh] flex flex-col items-center justify-center gap-3 text-center px-4">
        ...
        <h1 className="text-xl font-black" style={{ color: INK }}>
          Store not found
        </h1>
        <Link
          href="/shop"
  ...
  const payments = Array.isArray(store.paymentMethods)
    ? (store.paymentMethods as string[])
    : [];
  const memberYear = data.memberSince
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm -mt-14 relative p-6 md:p-8">
          <div className="flex flex-col md:flex-row md:items-start gap-5">
            <div
              ...
              style={{
                background: `linear-gradient(135deg, ${VIOLET}, #FF2E9A)`,
              }}
            >
              {store.logoUrl ? (
                <img
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge
                  ...
                  label="Secure payouts active"
                  color="#10B981"
                />
                {data.hasPhysicalStore && (
                  <Badge icon={<Store size={12} />} label="Physical store" />
                )}
                {store.status === "paused" && (
                  <Badge
                    icon={<Clock size={12} />}
              ...
              <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-3 text-sm text-gray-600">
                <span className="inline-flex items-center gap-1 font-bold">
                  ...
                  {reviewsQ.data && reviewsQ.data.length > 0 && (
                    <span className="text-gray-400 font-normal">
                      ({data.trustMetrics.verifiedReviews} reviews)
                    </span>
                  )}
                </span>
                ...
                {memberYear && (
                  <span className="inline-flex items-center gap-1">
                    <Clock size={14} /> Member since {memberYear}
                  </span>
                )}
                <span className="inline-flex items-center gap-1">
                  <BadgeCheck size={14} /> {data.trustMetrics.verifiedReviews}{" "}
                  verified reviews
                </span>
              </div>
            </div>
            ...
            >
              <Flag size={14} /> Report store
            </Button>
          </div>

          {store.description && (
            <p className="text-sm text-gray-600 leading-relaxed mt-5 border-t border-gray-100 pt-5">
              {store.description}
            </p>
          )}
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-6">
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            <div
              className="flex items-center gap-2 text-xs font-black uppercase tracking-wide mb-3"
              style={{ color: VIOLET }}
            >
            ...
            <div className="flex flex-wrap gap-2">
              {payments.length > 0 ? (
                payments.map(p => (
                  <span
                    key={p}
                    className="inline-flex items-center gap-1.5 text-xs font-bold bg-gray-100 rounded-full px-3 py-1.5 text-gray-700"
                  >
            ...
            {payments.includes("card") && (
              <p className="text-[11px] text-gray-400 mt-3 flex items-center gap-1">
                <LockKeyhole size={11} /> Card payments protected by Stripe +
                buyer protection
              </p>
            )}
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-5">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              {store.shippingPolicy ??
                "Contact the seller for shipping details."}
            </p>
            <p className="text-[11px] text-gray-400 mt-2">
              Ships in {store.handlingDays}{" "}
              {store.handlingDays === 1 ? "day" : "days"}
        ...
        <div className="mt-10">
          <h2
            ...
            style={{ color: INK, fontFamily: "var(--font-display)" }}
          >
            <Package size={18} style={{ color: VIOLET }} /> Singles for sale{" "}
                ...
                >
                  ...
                  <div className="p-2.5">
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      <span className="text-[10px] font-bold bg-gray-100 rounded px-1.5 py-0.5 text-gray-600">
        ...
        {products.length > 0 && (
          <div className="mt-10">
            <h2
              ...
              style={{ color: INK, fontFamily: "var(--font-display)" }}
            >
              <ShoppingBag size={18} style={{ color: VIOLET }} /> Sealed
              products{" "}
              <span className="text-gray-400 font-bold text-sm">
                ({products.length})
              </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {products.map(p => (
                <Link
                  key={p.listing.id}
                  href={p.productSlug ? `/shop/${p.productSlug}` : "/shop"}
                  className="group bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all"
                >
                  {p.productImageUrl && (
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-xs font-bold" style={{ color: INK }}>
                      {r.reviewerName ?? r.reviewerUsername ?? "Buyer"}
                    </span>
                    {r.review.orderId && (
                    ...
                    <span className="ml-auto inline-flex">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          key={i}
                          size={12}
                          fill={i < r.review.rating ? GOLD : "none"}
                          style={{
                            color: i < r.review.rating ? GOLD : "#d1d5db",
                          }}
                        />
                      ))}
                    </span>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Flag className="h-5 w-5 text-red-600" /> Report a safety concern
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Reports are private and reviewed by RarityGrid. Do not include
              payment card or identity-document numbers.
            </p>
            ...
            <label className="block space-y-1.5 text-sm font-bold">
              ...
              <Textarea
                ...
                rows={5}
                maxLength={2000}
                placeholder="Describe the listing, product or behavior that should be reviewed."
              />
              <span className="block text-right text-xs font-normal text-gray-400">
                {reportDetails.trim().length}/2000 · minimum 20 characters
              </span>
            </label>
            <Button
              variant="destructive"
              className="w-full"
              disabled={reportDetails.trim().length < 20 || report.isPending}
              onClick={() =>
                report.mutate({
                  storeId: store.id,
                  reason: reportReason,
            ...
            >
              {report.isPending ? "Submitting…" : "Submit private report"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>

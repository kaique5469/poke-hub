...
export default function SetDetail() {
  ...
  const { data, isLoading, error } = trpc.sets.detail.useQuery(
    { id },
    { enabled: Boolean(id), retry: false }
  );
  usePageMeta(
    data?.set?.name ?? "Pokémon TCG Set",
    data?.set
      ? `Complete ${data.set.name} card list, sealed products, release information and market highlights.`
      : "Cards, sealed products and market highlights for this Pokémon TCG expansion.",
    data?.set?.images?.logo,
    {
      structuredData: data?.set
        ? {
            "@context": "https://schema.org",
            "@type": "CollectionPage",
            url: `https://raritygrid.com/sets/${id}`,
            name: `${data.set.name} Pokémon TCG Set`,
            description: `Complete ${data.set.name} card list, sealed products, release information and market highlights.`,
            image: data.set.images.logo,
            datePublished: data.set.releaseDate,
            isPartOf: {
              "@type": "WebSite",
              name: "RarityGrid",
  ...
  if (error || !data) {
    return (
      <div className="container py-24 text-center">
        ...
        <Link
          href="/sets"
          className="inline-flex items-center gap-2 mt-5 text-primary font-bold"
        >
          <ArrowLeft size={16} /> All sets
        </Link>
      </div>
    );
  }
  ...
  const releaseDate = set.releaseDate
    ? new Date(
        `${set.releaseDate.replaceAll("/", "-")}T12:00:00`
          ...
          <div className="relative grid md:grid-cols-[1fr_360px] gap-8 items-center p-7 md:p-12 min-h-64">
            <div>
              <div className="flex flex-wrap gap-2 mb-5">
                <span className="rounded-full bg-white/10 border border-white/15 px-3 py-1 text-xs font-bold">
                  {set.series}
                </span>
                <span className="rounded-full bg-emerald-400/15 border border-emerald-300/20 px-3 py-1 text-xs font-bold text-emerald-200">
                  English · USD market
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 mt-5 text-sm text-white/75">
                {releaseDate && (
                  <span className="inline-flex items-center gap-2">
                    <CalendarDays size={16} /> {releaseDate}
                  </span>
                )}
                <span className="inline-flex items-center gap-2">
                  <CreditCard size={16} /> {set.total} cards
                </span>
                <span className="inline-flex items-center gap-2">
                  <Package size={16} /> {products.length} sealed products
                </span>
              </div>
            </div>
        ...
        <section>
          <div className="flex items-end justify-between gap-4 mb-5">
            <div>
              ...
              <h2 className="font-display text-2xl md:text-3xl font-black text-slate-900 mt-1">
                Sealed products
              </h2>
              <p className="text-sm text-slate-500 mt-1">
                Official product photography and current USD reference prices.
              </p>
            </div>
            <Link
              href={`/shop?set=${set.id}`}
              className="hidden sm:inline-flex items-center gap-2 text-sm font-bold text-violet-700 hover:text-violet-900"
            >
              ...
              {products.map((product: any) => {
                ...
                return (
                  <Link
                    ...
                    className="group rounded-2xl bg-white border border-slate-200 overflow-hidden hover:-translate-y-1 hover:shadow-xl hover:border-violet-300 transition-all"
                  >
                    <div className="relative aspect-square bg-gradient-to-b from-white to-slate-50 p-5 flex items-center justify-center">
                      <span className="absolute left-3 top-3 rounded-full bg-slate-900/85 text-white text-[10px] font-black px-2.5 py-1">
                        {productTypeLabel(product.category)}
                      </span>
                      {product.imageUrl ? (
                        <img
        ...
        {hotCards.length > 0 && (
          <section>
            <div className="flex items-center gap-3 mb-5">
              <span className="grid place-items-center h-9 w-9 rounded-xl bg-orange-100 text-orange-600">
                <Flame size={18} />
              </span>
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Chase cards and standout rarities from {set.name}.
                </p>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="relative rounded-xl overflow-hidden shadow-md group-hover:-translate-y-1 group-hover:shadow-xl transition-all bg-slate-900">
                      ...
                      />
                      {price !== null && (
                        <span className="absolute bottom-2 right-2 rounded-lg bg-black/80 px-2 py-1 text-xs font-black text-emerald-300">
                          ${price.toFixed(2)}
                        </span>
                      )}
                    </div>
            ...
            {cards.map((card: any) => (
              ...
              >
                <img
                  ...
                  alt={card.name}
                  loading="lazy"
                  className="w-full rounded-lg shadow-sm group-hover:-translate-y-1 group-hover:shadow-lg transition-all"
                />
                <p className="mt-1.5 text-[11px] font-bold text-slate-700 truncate">
                  {card.name}
                </p>

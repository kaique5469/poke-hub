...
function lineInfo(row: CartRow) {
  const unit = Number(
    row.cardListing?.priceUsd ?? row.productListing?.priceUsd ?? 0
  );
  ...
  const href = row.cardListing
    ? `/cards/${row.cardListing.cardId}`
    : row.productSlug
...
export default function Cart() {
  ...
  const { data: rows, isLoading } = trpc.cart.get.useQuery(undefined, {
  ...
  const groups = useMemo(() => {
    const map = new Map<
      number,
      { seller: CartRow["seller"]; rows: CartRow[] }
    >();
  ...
  const total = useMemo(
    () =>
      ((rows ?? []) as CartRow[]).reduce(
        (sum, r) => sum + lineInfo(r).unit * r.item.quantity,
        0
      ),
    [rows]
  );
                  ...
                  {g.rows.map(r => {
                    ...
                    return (
                      ...
                      >
                        <Link
                          href={info.href}
                          className="w-12 h-16 rounded bg-gray-50 flex items-center justify-center shrink-0 overflow-hidden"
                        >
                          {info.img ? (
                            <img
                              src={info.img}
                        ...
                        <div className="flex-1 min-w-0">
                          <Link
                            href={info.href}
                            className="font-bold text-sm line-clamp-1 hover:underline"
                            style={{ color: "oklch(0.18 0.02 240)" }}
                          >
                            {info.name}
                          </Link>
                          <div className="flex items-center gap-2 mt-1">
                            {info.condition && (
                              <ConditionPill condition={info.condition} />
                            )}
                            {r.cardListing?.isFoil && (
                            ...
                            <span
                              ...
                              style={{ color: "oklch(0.52 0.015 240)" }}
                            >
                              {formatMarketplaceMoney(info.unit)} cada ·{" "}
                              {info.stock} em estoque
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 shrink-0">
                          <button
                            className="page-btn w-7 h-7 p-0 flex items-center justify-center"
                            disabled={update.isPending}
                            onClick={() =>
                              update.mutate({
                                cartItemId: r.item.id,
                          ...
                          <button
                            className="page-btn w-7 h-7 p-0 flex items-center justify-center disabled:opacity-40"
                            disabled={
                              update.isPending || r.item.quantity >= info.stock
                            }
                            onClick={() =>
                              update.mutate({
                                cartItemId: r.item.id,
                        ...
                        <div
                          className="w-20 text-right font-black text-sm shrink-0"
                          style={{ color: "oklch(0.18 0.02 240)" }}
                        >
                          {formatMarketplaceMoney(info.unit * r.item.quantity)}
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 sticky top-20">
              ...
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-bold">
                  {cartRows.reduce((s, r) => s + r.item.quantity, 0)}
                </span>
              </div>
              <div className="flex justify-between text-sm mb-1">
              ...
              <div className="flex justify-between text-sm mb-1">
                <span style={{ color: "oklch(0.52 0.015 240)" }}>
                  Frete rastreado no Brasil
                </span>
                <span className="font-bold text-emerald-600">Incluído</span>
              </div>
              <div
                className="h-px my-3"
              ...
              <Textarea
                ...
                maxLength={1000}
                placeholder="Observação ao vendedor (opcional)..."
              ...
              <label className="mb-3 flex cursor-pointer items-start gap-2.5 rounded-lg border border-gray-200 p-3 text-xs text-gray-600">
                ...
                <span>
                  ...
                  , incluindo regras de condição, envio e proteção ao comprador.
                </span>
              </label>
              ...
              {groups.length > 1 && (
                <p className="mb-3 rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-900">
                  Para sua segurança, finalize um vendedor por vez. Remova os
                  itens dos demais vendedores antes de pagar.
                </p>
              )}
              {!stripeAvailable.isLoading && !stripeAvailable.data && (
                <p className="rounded-lg bg-amber-50 p-3 text-xs font-semibold text-amber-900">
                  O checkout seguro está temporariamente indisponível. Nada foi
                  cobrado ou reservado.
                </p>
              )}
              ...
              <div className="flex items-center justify-center gap-4 mt-4 pt-3 border-t border-gray-100 text-[10px] font-bold text-gray-400">
                <span className="inline-flex items-center gap-1">
                  <LockKeyhole size={11} /> SSL encrypted
                </span>
                <span className="inline-flex items-center gap-1">
                  <CreditCard size={11} /> Stripe secure
                </span>
                <span className="inline-flex items-center gap-1">
              ...
              <p
                ...
                style={{ color: "oklch(0.52 0.015 240)" }}
              >
                O estoque fica reservado por 30 minutos. O Stripe recebe os
                dados do cartão, Pix e endereço; o RarityGrid não armazena
                números de cartão nem CPF.
              </p>
            </div>
          </div>
        )}
